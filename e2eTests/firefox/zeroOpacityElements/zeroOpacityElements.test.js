    ...
    it('set zero opacity elements from webpage and compares', () => {
        ...
        try {
            const snapOutput = execSync(
                'node ./lib/bin/run.js snap --browser firefox --config e2eTests/firefox/zeroOpacityElements/zeroOpacityElementsConfig.json'
            ).toString();
            console.log(snapOutput);
            const compareOutput = execSync(
                'node ./lib/bin/run.js compare --browser firefox --config e2eTests/firefox/zeroOpacityElements/zeroOpacityElementsConfig.json'
            ).toString();
