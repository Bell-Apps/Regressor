...
import fs from 'fs';
import config from '../../chrome/removeElements/removeElementsConfig.json';
    ...
    it('remove elements from webpage and compares', () => {
        ...
        try {
            const snapOutput = execSync(
                'node ./lib/bin/run.js snap --browser firefox --config e2eTests/firefox/removeElements/removeElementsConfig.json'
            ).toString();
            console.log(snapOutput);

            const compareOutput = execSync(
                'node ./lib/bin/run.js compare --browser firefox --config e2eTests/firefox/removeElements/removeElementsConfig.json'
            ).toString();
